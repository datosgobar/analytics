...
@click.command()
def log_cleaner():
    log_csv_path = './data/api-series.csv'
    log_csv_path_clean = './data/api-series-clean.csv'
    ...
    with open('./data/api-series.log') as log:
    ...
    for df_ in reader:
        ...
        df.loc[:, 'querystring'] = request.apply(lambda x: x[1].split('?')[-1]).copy()
